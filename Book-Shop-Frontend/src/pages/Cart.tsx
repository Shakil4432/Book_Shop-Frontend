...
import {
  ...
  removeFromCart,
  updateQuantity,
} from "../redux/features/userManagement/cartSlice";
...
import { useGetUserByIdQuery } from "../redux/features/userManagement/userApi";

const Cart = () => {
  const token = useAppSelector(useCurrentToken);
  const currentUser = useAppSelector(selectCurrentUser);
  const userWithId = useGetUserByIdQuery(currentUser?.email);
  ...
  const handlePlaceOrder = async () => {
    try {
      const orderData = {
        userId: userWithId?.data?.data?._id,
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "price",
      render: (price: number) => (
        <span className="text-[#e7995e] font-medium">${price}</span>
      ),
    },
    {
      title: "Quantity",
      ...
      render: (text: number, record: any) => (
        <input
          ...
          onChange={(e) =>
            handleQuantityChange(record._id, Number(e.target.value))
          }
        />
      ),
    },
    {
      ...
      key: "total",
      render: (record: any) => (
        <span className="text-[#e7995e] font-medium">
          ...
          >
            <div className="text-center md:text-left">
              <h2 className="text-lg font-semibold text-gray-700">
                Total Items:{" "}
                <span className="text-[#e7995e]">{totalQuantity}</span>
              </h2>
              <h2 className="text-lg font-semibold text-gray-700">
                Total Price:{" "}
                <span className="text-[#e7995e]">${totalToFixed}</span>
              </h2>
            </div>
